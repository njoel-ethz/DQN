...
def main():
    path_indata = 'measurements'

    for input_file in os.listdir(path_indata):
        # unzip
        if input_file.endswith(".csv"):
            name = input_file.split('.')[0]
            reward_file = os.path.join(path_indata, name + '.png')
            averaged_reward_file = os.path.join(path_indata, name + '_averaged.png')

            rewards = [float(row[0]) for row in csv.reader(open(os.path.join('measurements', input_file)))]
            averaged_rewards = []
            temp_sum = 0
            index_stats = []
            for i in range(len(rewards)):
                ...
                if i%10==0 and i>1:
                    ...
                    averaged_rewards.append(temp_sum)
                    temp_sum = 0
